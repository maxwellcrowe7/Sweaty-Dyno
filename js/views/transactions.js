...
export function render(db, state = {}) {
  ...
  <div class="view-hd"><h2>Transactions</h2>
    ...
    <div class="hd-picks">${mgrPicker(db, S, mgr ?? '')}${seasonPicker(db)}</div></div>
  ...
  root.querySelectorAll('[data-cond-edit]').forEach((b) => b.addEventListener('click', () => {
    ...
    m.root.querySelectorAll('[data-col]').forEach((col) => {
      ...
      col.querySelector('[data-pal-open]').addEventListener('click', (e) => {
        e.stopPropagation();
        const pal = col.querySelector('[data-pal]');
        const wasOpen = pal.classList.contains('open');
        m.root.querySelectorAll('.ab-pal').forEach((x) => x.classList.remove('open'));
        // the trigger toggles: clicking it again puts the options away
        if (!wasOpen) pal.classList.add('open');
      });
