  ...
  openModal({
    ...
    body: `
      ...
      <div class="fgrid">
        <div class="field"><label>Their name</label>
          <input name="name" placeholder="First name" value="${esc(suggested.split(/\s+/)[0] || '')}"></div>
        <div class="field"><label>Full name</label><input name="fullName" placeholder="First Last"></div>
      </div>
      <div class="fgrid">
    ...
    onConfirm: async (d) => {
      ...
      if (!id) {
        ...
        if (!name) { toast('Name the incoming manager'); return false; }
        id = name.toLowerCase().replace(/[^a-z0-9]+/g, '') || `mgr${Date.now()}`;
      }
      await db.update('managers', (m) => {
        if (!m.managers.some((x) => x.id === id)) {
          m.managers.push({
            ...
            aliases: [String(d.name || '').trim(), String(d.fullName || '').trim(),
              String(d.sleeper || '').trim()].filter(Boolean),
            sleeperUserId: null,
            sleeperUsername: String(d.sleeper || '').trim() || null,
            usernameConfirmed: false,
            fullName: String(d.fullName || '').trim() || String(d.name || '').trim() || id,
          });
        }
